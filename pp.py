...
GOOGLE_API_KEY = "AIzaSyAbu3uErPECsNdKxJ6AIEQ1UXmGwG1WlCI"  # Replace with your actual API Key  <--- IMPORTANT!
...
if db is None:
    ...
    def generate_response(full_query):
        try:
            ...
            st.error(f"An error occurred during response generation: {e}")  # Display error in Streamlit
            return "An error occurred. Please try again."  # Return a user-friendly message
    ...
    if input := st.chat_input("Enter your question about the PDFs..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Searching for answers..."):
                ...
                for message in st.session_state.messages[-5:]:  # Context window: last 5 messages
                    conversation_history += f"{message['role']}: {message['content']}\n"

                full_query = f"{conversation_history}User: {input}"

                response = generate_response(full_query)
                st.write(response)
